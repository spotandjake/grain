...
module Pat = {
  ...
  let list = (~loc, a) => {
    let empty = construct(~loc = loc, ident_empty, []);
    let a = List.rev(a);
    switch (a) {
    | [] => empty
    | [base, ...rest] =>
      let base =
        switch (base) {
        | ListItem(pat) => construct(~loc = pat.ppat_loc, ident_cons, [pat, empty])
        | ListSpread(pat, _) => pat
        };
      List.fold_left(
        (acc, pat) => {
          switch (pat) {
          | ListItem(pat) => construct(~loc = pat.ppat_loc, ident_cons, [pat, acc])
          | ListSpread(_, loc) =>
            raise(
              SyntaxError(
                loc,
    ...
    let list =
      ...
      | [base, ...rest] =>
        let base =
          switch (base) {
          | ListItem(expr) =>
            construct(~loc = expr.pexp_loc, ~attributes?, ident_cons, [expr, empty])
        ...
        List.fold_left(
          (acc, expr) => {
            switch (expr) {
            | ListItem(expr) =>
              construct(~loc = expr.pexp_loc, ~attributes?, ident_cons, [expr, acc])
            | ListSpread(_, loc) =>
              raise(
                SyntaxError(
                  loc,
